...
class MAX30100(object):
    ...
    def set_mode(self, mode):
        # Direct write, no read (prevents timeout)
        self.write_byte_data(I2C_ADDRESS, MODE_CONFIG, 0x00)
        self.write_byte_data(I2C_ADDRESS, MODE_CONFIG, mode)
        
    def set_spo_config(self, sample_rate=100, pulse_width=1600):
        sr = _get_valid(SAMPLE_RATE, sample_rate)
        pw = _get_valid(PULSE_WIDTH, pulse_width)
        value = (sr << 2) | pw
        self.write_byte_data(I2C_ADDRESS, SPO2_CONFIG, value)
    ...
    def write_byte_data(self, i2c_addr, addr, value):
        if isinstance(value, str):
            value = ord(value)   # convert char → int
        self.i2c.writeto_mem(i2c_addr, addr, bytes([value]))
